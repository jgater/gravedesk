...
var saveMail = function(mail,id) {
    ...
    if (mail.attachments) {
		...
		ticketdb.ticketFromEmail(procmail,function(err){
				if (err) {
...
var sendMail = function(mail,id,callback){
    ...
    if(err){
        ...
        console.log("Message sent for ticket ID: "+ id + ", server response: " + response.message);
        // if mail sent successfully, find ticket by id and add sent mail to history
        async.waterfall([
        	function(callback) {
        		ticketdb.findById(id,callback);
        	},
        	function(ticket,callback) {
        		mail.date = new Date();
        		ticket.emails.push(mail);
        		ticketdb.updateTicketById(id,ticket,callback);
        	}
        ], function(err,result) {
        	if (err) {
        		console.error(err);
