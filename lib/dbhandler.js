  ...
  this.newTicket = function(params,callback) {
	...
	this.attachmentStubs = function(attachments,index,callback) {
		...
		var iterator = function(item,cb){
					var stub = {};
					stub.date = date;
					stub.fileName = index+"_"+item.fileName;
	...
	this.saveAttachments = function(attachments,index,id) {
		id = id+'';
		index = index+'';
		fs.stat(settings.attachmentDir, function(err,stats){
			if (!stats || err) {
				fs.mkdirSync(settings.attachmentDir);
				}
				fs.stat(path.join(settings.attachmentDir, id), function(err,stats){
					if (!stats || err) {
						fs.mkdirSync(path.join(settings.attachmentDir,id));
					}

					for (var i in attachments) {
						var base64Data = attachments[i].content;
						var dataBuffer = new Buffer(base64Data, 'base64');
						var filePath = path.join(settings.attachmentDir, id, index+"_"+attachments[i].fileName);	
						fs.writeFile(filePath, dataBuffer, function(err) {
							if (err) {
  							console.log(err);
  						} else {
  							console.log(filePath+" written to disk.");
  						}
  					});
					}
