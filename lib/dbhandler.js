...
var TicketProvider = function() {
  ...
  this.newTicket = function(params,callback) {
  	var self=this;
		...
		self.attachmentStubs(attachments,function(err,stubs){
			if (!err) {
				var ticket = new ticketmodel();
				params.attachments = stubs;
				ticket.attachments = stubs;
				...
				ticket.save(function(err,doc){
					if (err) {callback(err);
						...
						self.saveAttachments(attachments,doc._id);
	...
	this.attachmentStubs = function(attachments,callback) {
		var date = new Date();
		var iterator = function(item,cb){
					var stub = {};
					stub.date = date;
					stub.fileName = item.fileName;
					stub.contentType = item.contentType;
					cb(null,stub);
		};
		async.map(attachments, iterator, callback);
	},

	this.saveAttachments = function(attachments,id) {
		for (var i in attachments) {
			var base64Data = attachments[i].content;
			var dataBuffer = new Buffer(base64Data, 'base64');	
			fs.mkdirSync('attachments');
			fs.mkdirSync('attachments/'+id);
			var path = 'attachments/'+id+'/'+attachments[i].fileName;
	...
	this.updateTicketEmailsById = function(id, ticket, callback) {
		...
		var update = { attachments: ticket.attachments, emails: ticket.emails, lastmodified: new Date() };
		var options = {};
